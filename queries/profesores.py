...
def qw_get_profesores():
    try:
        ...
        if len(profesores) == 0:
            Logger.warning(f"No se han encontrado a los profesores. {profesores}", "./logs/logs_profesores.txt")
            return JSONResponse(content={"message": "Error: No Existe la tabla"}, status_code=404)
        
    except Exception as e:
        Logger.error(f"Error {profesores} al consultar profesores.", "./logs/logs_profesores.txt")
        return JSONResponse(content={"message": f"No se ha encontrado la lista de profesores.{e}"}, status_code=400)
    Logger.info(f"La lista de profesores ha sido encontrada.{profesores}", "./logs/logs_profesores.txt")
...
def qw_list_profesores(nombre_profe):
    try:            
        ...
        if profesores is None:
            Logger.warning(f"El profesor/a especificado no existe {profesores}", "./logs/logs_profesores.txt")
            return JSONResponse(content={"message": "Error: El profesor/a especificado no existe."}, status_code=404)
    except Exception as e:
        Logger.error(f"Error {profesores} al consultar el profesor.", "./logs/logs_profesores.txt")
        return JSONResponse(content={"message": f"No se ha podido encontrar el profesor {e}"}, status_code=400)
    Logger.info(f"El profesor ha sido encontrado. {profesores}", "./logs/logs_profesores.txt")
...
def qw_post_profesores(datos_profesor):
    try:
        ...
        if profesor_existe is not None:
            Logger.warning(f"El profesor/a ya existe. {profesor}", "./logs/logs_profesores.txt")
            return JSONResponse(content={"message": "El profesor/a ya existe."}, status_code=404)
        session.add(profesor)
        session.flush()
        session.commit()    
    except Exception as e:
        Logger.error(f"El profesor no ha podido ser grabado. {profesor}", "./logs/logs_profesores.txt")
        return JSONResponse(content={"message": f"El profesor no ha podido ser grabado.{e}"}, status_code=400)
    Logger.info(f"El profesor/a ha sido grabado. El/la {profesor}", "./logs/logs_profesores.txt")
...
def qw_put_profesores(nombre_profesor, nuevo_profesor):
    try:
        ...
        if profesor is None:
            Logger.warning(f"El profesor ya existe. {profesor}", "./logs/logs_profesores.txt")
        ...
        Logger.error(f"No se ha podido añadir el profesor/a. {profesor}", "./logs/logs_profesores.txt")
        return JSONResponse(content={"message": f"No se ha podido añadir el profesor/a.{e}"}, status_code=400)
    Logger.info(f"El profesor/a ha sido modificado. {profesor}", "./logs/logs_profesores.txt")
...
def qw_delete_profesores(borrar_profesor):
    try:
        ...
        if profesor is None:
            Logger.warning(f"El profesor especificado no existe. {profesor}", "./logs/logs_profesores.txt")
            return JSONResponse(content={"message": "Error: El profesor especificado no existe."}, status_code=404)
        session.delete(profesor)
        session.commit()
  
    except Exception as e:
        Logger.error(f"No se ha podido borrar el profesor/a: {profesor}", "./logs/logs_profesores.txt")
        return JSONResponse(content={"message": f"No se ha podido borrar el profesor/a.{e}"}, status_code=400)
    Logger.info(f"El profesor/a ha sido borrado. {profesor}", "./logs/logs_profesores.txt")
