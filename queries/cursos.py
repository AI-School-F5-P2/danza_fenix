...
def qw_create_curso(curso_input):
    try:
        ...
        if curso_existe is not None:
            Logger.error(f"El curso {curso_input['nombre_curso']} se ha intentado grabar, pero ya existía", "./logs/logs_cursos.txt")
        ...
        session.rollback()
        Logger.error(f"No se ha podido crear el curso {curso_input['nombre_curso']}.{e}", "./logs/logs_cursos.txt")
        return JSONResponse(content={"message": f"No se ha podido crear el curso.{e}"}, status_code=400)
    Logger.info(f"El curso {curso_input['nombre_curso']} ha sido creado.", "./logs/logs_cursos.txt")
...
def qw_get_cursos():
    ...
    if len(cursos) == 0:
        Logger.error("No se han encontrado cursos.", "./logs/logs_cursos.txt")
    ...
    Logger.info("Se han encontrado cursos.", "./logs/logs_cursos.txt")   
    return cursos_dict
...
def qw_mostrar_curso(nombre_del_curso):
    ...
    if curso is None:
        Logger.error(f"El curso {nombre_del_curso} no existe.", "./logs/logs_cursos.txt")
...
def qw_get_curso_unique(nombre_del_curso):
    try:
        ...
        Logger.error(f"No se ha podido mostrar el curso {nombre_del_curso}.{e}", "./logs/logs_cursos.txt")
        return JSONResponse(content={"message": f"No se ha podido mostrar el curso.{e}"}, status_code=400)
    Logger.info(f"Se ha encontrado el curso {nombre_del_curso}.", "./logs/logs_cursos.txt")
    return curso
...
def qw_update_curso(nombre_del_curso, nuevo_nombre, nuevo_precio, nuevo_grupo):
    try:
        curso = session.query(Curso).filter(Curso.nombre_curso == nombre_del_curso).first()
        if curso is None:
            Logger.error(f"El curso  no existe.", "./logs/logs_cursos.txt")
            return JSONResponse(content={"message": "Error: El curso especificado no existe."}, status_code=404)
        ...
        Logger.error(f"No se ha podido modificar el curso {nombre_del_curso}.{e}", "./logs/logs_cursos.txt")   
        return JSONResponse(content={"message": f"No se ha podido modificar el curso.{e}"}, status_code=400)
    Logger.info(f"El curso {nombre_del_curso} ha sido modificado.", "./logs/logs_cursos.txt")
...
def qw_delete_curso(nombre_del_curso):
    try:
        ...
        Logger.error(f"No se ha podido borrar el curso {nombre_del_curso}.{e}", "./logs/logs_cursos.txt")
        return JSONResponse(content={"message": f"No se ha podido borrar el curso.{e}"}, status_code=400)
    Logger.info(f"El curso {nombre_del_curso} ha sido borrado.", "./logs/logs_cursos.txt")
