...
def qw_get_alumnos():
    try:            
        ...
        if len(alumnos) == 0:
            Logger.info(f"no se han encontrado alumnos.  {alumnos}", "./logs/logs_alumnos.txt")
            return JSONResponse(content={"message": f"no se han encontrado alumnos. {e}"}, status_code=404)
        else:
            return alumnos
    except Exception as e:
        Logger.error(f"error {alumnos} al consultar alumnos.", "./logs/logs_alumnos.txt")
...
def qw_get_alumno(dni_alumno):
    try:            
        ...
        if alumno is None:
            Logger.info(f"no se han encontrado alumno.  {alumno}", "./logs/logs_alumnos.txt")
            return JSONResponse(content={"message": f"no se han encontrado alumno. {e}"}, status_code=404)
        else:
            return alumno
    except Exception as e:
        Logger.error(f"error {alumno} al consultar alumno.", "./logs/logs_alumnos.txt")
        return JSONResponse(content={"message": f"error al consultar alumnos. {e}"}, status_code=400) 
...
def qw_post_alumnos(datos_alumno):
    try:
        ...
        session.flush()
        session.commit()
        Logger.info(f"se han grabado los datos {datos_alumno}", "./logs/logs_alumnos.txt")
        return JSONResponse({"mensaje": "Alumno creado correctamente"}, status_code=200)
    except Exception as e:
        if str(type(e)) == "<class 'sqlalchemy.exc.IntegrityError'>":
            Logger.info(f"El alumno ya existe {datos_alumno}", "./logs/logs_alumnos.txt")
            return JSONResponse({"mensaje": "El alumno ya existe"}, status_code=401) 
        else:
            Logger.error(f"Error{datos_alumno} al insertar alumnos", "./logs/logs_alumnos.txt")
...
def qw_put_alumnos(dni_alumno,alumno):
    try:
        ...
        if data_alumno is None:
             Logger.info(f"se ha encontrado el alumno {dni_alumno,alumno}", "./logs/logs_alumnos.txt")
        ...
        session.flush()
        session.commit()
        Logger.info(f"se han grabado los datos {dni_alumno,alumno}", "./logs/logs_alumnos.txt")
        return JSONResponse({"mensaje": "Alumno actualizado correctamente"}, status_code=200)
    except Exception as e:
        if str(type(e)) == "<class 'sqlalchemy.exc.IntegrityError'>":
            Logger.info(f"El alumno no se pudo actualizar {dni_alumno,alumno}", "./logs/logs_alumnos.txt")
            return JSONResponse({"mensaje": "El alumno no se pudo actualizar"}, status_code=400)
        else:
            Logger.error(f"error {dni_alumno,alumno} al actualizar alumno", "./logs/logs_alumnos.txt")
...
def qw_delete_alumno(dni_eliminar_alumno):
    try:
        ...
        if eliminar_alumno is None:
            Logger.info(f"el alumno no se encontro {dni_eliminar_alumno}", "./logs/logs_alumnos.txt")
            return JSONResponse({"mensaje": "El alumno no se encontro"}, status_code=404)
        session.delete(eliminar_alumno)
        session.commit()
        Logger.info(f"Alumno eliminado correctamente {dni_eliminar_alumno}", "./logs/logs_alumnos.txt")
        return JSONResponse({"mensaje": "Alumno eliminado correctamente"}, status_code=200)
    except Exception as e:
        Logger.error(f"error {dni_eliminar_alumno} al eliminar alumno", "./logs/logs_alumnos.txt")
