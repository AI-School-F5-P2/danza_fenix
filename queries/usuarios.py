...
def qw_create_usuario(usuario):
    ...
    if login_existe:
        Logger.error(f"Se ha intentado grabar el usuario {usuario.login}, pero ya existe.")
        return "El login ya existe."
    email_existe = session.query(Usuario).filter(Usuario.email == usuario.email).first()
    if email_existe:
        Logger.error(f"Se ha intentado grabar el email {usuario.email}, pero ya existe.")
    ...
    Logger.info(f"Se ha grabado el usuario {usuario.login}.")
...
def qw_list_usuarios():
    ...
    Logger.info("Se han listado los usuarios.")
    return result
...
def qw_show_usuario(dato, valor):
    ...
    if not resultado:
        Logger.error("Se ha intentado ver un usuario que no existe.")
    ...
    Logger.info(f"Se ha visto el usuario {usuario.login}")
    return usuario_dict
...
def qw_update_usuario(dato, valor, usuario):
    ...
    if not rol:
        Logger.error(f"Se ha intentado actualizar el usuario {valor}, pero no existe.")
        return "El rol especificado no existe."
    ...
    session.flush()
    session.commit()
    Logger.info(f"Se ha actualizado el usuario {valor}")
...
def qw_delete_usuario(dato, valor):
    if dato == "id":
        ...
        return "Dato no válido."
    if not usuario:
        Logger.error(f"El usuario {valor} se ha intntado eliminar, pero no existe.")
        return "No se ha encontrado el usuario."
    session.delete(usuario)
    session.flush()
    session.commit()
    Logger.info(f"Se ha eliminado el usuario {valor}.")
