...
def qw_create_grupo(grupo_input):
    ...
    if grupo.nombre_grupo.strip() == "":
        Logger.error(f"Se ha intentado grabar un grupo, pero el nombre estaba vacío.")
    ...
    if grupo_existe is not None:
        Logger.error(f"Se ha intentado grabar el grupo {grupo.nombre_grupo}, pero ya existe previamente.")
        return "El grupo ya existe previamente."
    session.add(grupo)
    session.flush()
    session.commit()
    Logger.info(f"Se ha grabado el grupo {grupo.nombre_grupo}")
    return "El grupo ha sido grabado."

# Localizar un grupo por su id o nombre
def qw_get_grupo(dato, valor):
    if str(valor).strip() == "":
        Logger.error("Se ha intentado obtener un log con el valor vacío.")
    ...
    if dato == "id":
        ...
        if grupo is None:
            Logger.error(f"El grupo {valor} no existe")
            return "Error: El grupo especificado no existe."
    else:
        return "El tipo de dato especificado no existe."
    Logger.info(f"Se han obtenido los datos del grupo {valor}.")
    return grupo
...
def qw_list_grupos():
    ...
    if len(grupos) == 0:
        Logger.error("Se han intentado listar los grupos, pero no los hay.")
    ...
    for grupo in grupos:
        ...
        Logger.info("Se han listado los grupos.")
    return grupos
...
def qw_update_grupo(dato, valor, nuevo_nombre):
    if dato == "id":
        ...
        Logger.error(f"Se ha intentado actualizar el grupo {valor}, pero no existe.")
        return "El tipo de dato especificado no existe."
    ...
    Logger.info(f"Se ha actualizado el grupo {valor}")
...
def qw_delete_grupo(dato, valor):
    if dato == "id":
        ...
        Logger.error(f"Se ha intentado borrar el grupo {valor}, pero no existe.")
        return "El tipo de dato especificado no existe."
    ...
    if cursos_con_grupo > 0:
        Logger.error(f"Se ha intentado borrar el grupo {valor}, pero tiene cursos asociados.")
    ...
    session.flush()
    session.commit()
    Logger.info(f"Se ha eliminado el grupo {valor}.")
